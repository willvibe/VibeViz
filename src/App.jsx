...
const App = () => {
    ...
    const toggleLang = (newLang) => { setLang(newLang); localStorage.setItem('app_lang', newLang); };
    ...
    const [apiKeyConfig, setApiKeyConfig] = useState(() => localStorage.getItem('gemini_api_key') || '');
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => { try { const parsed = new Function(`return ${editorCode}`)(); setManualOption(parsed); } catch(err){} }, 800);
    ...
    useEffect(() => {
        if (audioUrl !== defaultBgAudio && audioRef.current) {
            const autoPlayNewAudio = async () => { try { await audioRef.current.play(); setIsPlayingAudio(true); } catch (err) {} };
            autoPlayNewAudio();
        }
    }, [audioUrl]);
    ...
    useEffect(() => {
        ...
        if (rainState === 2) { rainAudioRef.current.volume = 0.3; rainAudioRef.current.play().catch(err => {}); } 
        else if (rainState === 3) { rainAudioRef.current.volume = 1.0; rainAudioRef.current.play().catch(err => {}); } 
        else { rainAudioRef.current.pause(); }
    }, [rainState]);

    const handleApiKeyChange = (e) => { const val = e.target.value; setApiKeyConfig(val); localStorage.setItem('gemini_api_key', val); };
    ...
    useEffect(() => { if (processedData && processedData.length > 10000) setIsRenderPaused(true); else setIsRenderPaused(false); }, [processedData]);
    ...
    useEffect(() => {
        ...
        if (availableColumns.length > 0 && prevColsRef.current !== colsStr) {
            ...
            if (processedData.length > 10000) { setXAxis(''); setYAxes([]); setChartType(''); setSmartSwitchMsg(t(`⚠️ 数据量过大(>10000条)，已清空选项。请手动选择后点击渲染按钮！`, `⚠️ >10000 rows. Options cleared. Select manually and click Render!`)); setTimeout(() => setSmartSwitchMsg(''), 6000); return; }
    ...
    const handleCopyCode = () => { try { const textArea = document.createElement("textarea"); textArea.value = editorCode; document.body.appendChild(textArea); textArea.select(); document.execCommand("copy"); textArea.remove(); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch (err) {} };

    const handleCopyData = () => { if (!processedData || processedData.length === 0) return; try { const header = availableColumns.join('\t'); const rows = processedData.map(row => availableColumns.map(c => { let val = row[c]; if (val === undefined || val === null) return ''; return String(val).replace(/\t|\n/g, ' '); }).join('\t')); const tsvContent = [header, ...rows].join('\n'); const textArea = document.createElement("textarea"); textArea.value = tsvContent; document.body.appendChild(textArea); textArea.select(); document.execCommand("copy"); textArea.remove(); setDataCopied(true); setTimeout(() => setDataCopied(false), 2000); } catch (err) {} };
